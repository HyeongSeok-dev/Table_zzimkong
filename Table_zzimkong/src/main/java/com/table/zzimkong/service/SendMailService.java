...
public class SendMailService {
	...
	public String sendIdAuthMail(MemberVO member) {
		...
		String content = "<a href='http://c5d2308t2.itwillbs.com/Table_zzimkong/MemberEmailAuth?email=" + member.getUser_email() + "&auth_code=" + auth_code + "'>"
	...
	public String sendPasswdAuthMail(MemberVO member) {
		...
		String content = "<a href='http://c5d2308t2.itwillbs.com/Table_zzimkong/MemberEmailAuth?id=" + member.getUser_id() + "&auth_code=" + auth_code + "'>"
				+ "이메일 인증을 수행하려면 이 링크를 클릭해 주세요!</a>";
		new Thread(new Runnable() {
			@Override
			public void run() {
