...
<script type="text/javascript">
	$(document).ready(function() {
		$('#cs_button_delete').click(function() {
		    if (confirm("삭제하시겠습니까?")) {
		        location.href='${pageContext.request.contextPath }/admin/cs/delete?cs_board_num=${replyBoard.cs_board_num}';
		    }
		});
	});
</script>
...
<body class="info_window">
	<section>
		<h3>1 : 1 답글</h3>
		<br><br>
		<form action="modifyPro" method="post" name="memberQnaForm" enctype="multipart/form-data">
			<table>
				<tr>
					<th>번호</th>
				...
				<tr>
					...
					<td colspan="4">
						<input type="text" placeholder="제목" name="cs_board_subject" value="${replyBoard.cs_board_subject}">
					</td>
				</tr>
				<tr>
					<th colspan="2">답변내용</th>
					<td colspan="4">
						<textarea rows="20" cols="80" name="cs_board_content" >${replyBoard.cs_board_content}</textarea>
					</td>
				</tr>
				<tr>
			...
			<input type="hidden" name="cs_board_num" value="${replyBoard.cs_board_num}">
			<input type="hidden" name="cs_board_category_user" value="${replyBoard.cs_board_category_user}">
			<input type="hidden" name="cs_board_category_main" value="${replyBoard.cs_board_category_main}">
			<input type="hidden" name="cs_board_category_sub" value="${replyBoard.cs_board_category_sub}">
