...
<!-- 				<td colspan="5"><input type="text" maxlength="5" size="50" value="(68902) 부산광역시 해운대구 센텀3로 20 (우동, 센텀호텔316,317호)" disabled></td> -->
			...
			<tr>
				...
				<td colspan="2">
					<select>
						<option <c:if test="${company.com_ad_grade eq 0}">selected</c:if>>0단계</option>
						<option <c:if test="${company.com_ad_grade eq 1}">selected</c:if>>1단계</option>
						<option <c:if test="${company.com_ad_grade eq 2}">selected</c:if>>2단계</option>
						<option <c:if test="${company.com_ad_grade eq 3}">selected</c:if>>3단계</option>
					</select>
				</td>
			</tr>
			<tr>
				<th>위생등급</th>
				<td colspan="2"><input type="text" value="${company.com_hyg_rank}등급 (<fmt:formatDate value="${company.com_hyg_start_date}" pattern="yy/MM/dd HH:mm"/>)" disabled></td>
				<th>등록일자</th>
